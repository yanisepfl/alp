// B3c — derives snapshot.allocations + snapshot.pools from real on-chain
// reads against the deployed ALPVault. Owns:
...
//     this every snapshot path; the underlying view calls are cheap but
//     should not fan out per consumer)
...
//
// Boot policy: the FIRST snapshot served can serve the mock fallback for
// the first ~5-10s while the priming read completes; subsequent snapshots
// use the cache. Defensive: if any read errors, fall back to last-known
// cached value, then to null (caller substitutes mock). Never crash the
// snapshot path.
...
export function startComposition(c: PublicClient, addr: `0x${string}`): void {
  ...
  // Prime the cache off the boot path so the first snapshot can already
  // serve real data when the FE arrives. Don't await — boot must not block
  // on chain RPCs.
  void refresh().catch((e) => {
    console.warn(`[vault-composition] initial prime failed: ${e instanceof Error ? e.message : String(e)}`);
  });

  // Indexer-driven invalidation. Every agent action that mutates basket
  // composition is folded by the indexer in (block, logIndex) order; we
  // drop the cache so the next snapshot pull rebuilds against fresh state.
...
export function getCompositionSnapshot(): CompositionSnapshot | null {
  ...
  // cache (or null if no cache yet); the next call gets the fresh value.
...
async function refresh(): Promise<CompositionSnapshot | null> {
  ...
  // so pool.pct against totalAssets is consistent with the headline TVL.
  let poolMetas: Pool[];
  ...
  // Per-position decomposition. Flat-list one promise per (pool, positionId);
  // adapter.getPositionAmountsAtPrice mirrors the contract's _poolValueWithIdle
  // path so the sum is identical (modulo the idle non-base attribution, which
  // we explicitly aggregate via USDC.balanceOf below).
  ...
  // (cbBTC, ETH-as-WETH, USDT) sometimes sit unallocated between rebalances
  // and ALPVault's poolValueExternal attributes them to the pool registered
  // as their valuation source. We need them in our own token aggregation
  // too, otherwise (sum of pools' pct) overshoots 100% — pools include
  // non-base idle but our token totals don't.
  ...
  try {
    const balances = await Promise.all([
      ...
      ...uniqueNonBaseTokens.map((t) =>
        // Native ETH isn't an ERC20 — calling balanceOf on 0x0 reverts. The
        // V4 ETH/USDC pool stores idle ETH as the vault's native balance,
  ...
  // Token totals in their own raw base units (USDC 6, USDT 6, ETH 18,
  // BTC 8). Sums position amounts across every pool, plus all idle balances
  // (USDC + non-base). This must mirror what poolValueExternal accounts for,
  // otherwise per-pool pct sums diverge from 100%.
  ...
  // pricing source. Stables (USDT) are treated as $1 — sufficient for
  // demo accuracy (CONTRACT.md doesn't pin USDT pricing). Tokens with no
  // USDC-paired pool fall through with $0 USD value (allocation pct then
  // under-counts, but the gauges still move with chain state).
  ...
  for (const [sym, raw] of tokenRaw) {
    ...
    if (sym === "USDC") {
      ...
      if (!px) {
        // No pricing source — fall through with 0 contribution. Documented
        // limitation; demo currently always has a USDC pair for every
        // tracked non-base token.
        continue;
      }
  ...
  // gauges stay mutually consistent. Mirrors ALPVault._marketTAV: idle
  // USDC + sum(poolValueExternal). Falls back to the token-derived total
  // if poolValues sum to 0 (e.g. all pools empty pre-deposit).
  ...
  // Order allocations by descending pct so the FE chip row reads naturally
  ...
  for (let i = 0; i < poolMetas.length; i++) {
    ...
    // colors the chip off `position.left`; matching the mock keeps the BTC
    // pool blue, the ETH pool purple, etc.
  ...
  // the FE pool list shape is stable across rebalances. apr/earned30d are
  // zero — idle USDC doesn't accrue fees in the vault.
...
// Mirrors ALPVault._convertToBase (contracts/src/ALPVault.sol L1299):
//   nonBaseIsToken0 → base = amount * (sqrtPriceX96 / 2^96)^2
//   else            → base = amount / (sqrtPriceX96 / 2^96)^2
// Implemented in bigint to keep precision over the wide dynamic range of
// (BTC 8 dec) × (sqrtPriceX96 ~Q64.96).
...
// Stable, descriptive slug. Combines the human-readable token pair with the
// fee tier so the FE caches keyed on slug stay distinct across pools that
// share a token pair (e.g. USDC/USDT 0.01% vs 0.05%). Stable across reboots
// — same poolKey always produces the same slug.
function poolSlugFor(poolKey: `0x${string}`, pm: Pool, sym0: TokenSymbol, sym1: TokenSymbol): string {
  ...
  // V4 dynamic-fee pools encode the dynamic flag with the high bit set
  // (0x800000 = 8388608); render as "dyn" rather than a junk numeric.
  const DYN_FEE = 0x800000;
  const feeStr = (pm.fee & DYN_FEE)
    ? "dyn"
    : Math.floor(pm.fee / 100).toString().padStart(3, "0");
  // poolKey suffix would be the strictest dedupe but is not human-readable;
  // the FE never sees this if the pair+fee tuple is already unique.
  void poolKey;
