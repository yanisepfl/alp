...
//   - either the B1 random-walk mock ticker, or the B3/B3b chain reader
//
// Chain mode (B3): reads sharePrice + tvl from the deployed ALPVault every
// 5s, samples a 30-point daily history at startup, and emits partial
// vault.tick frames when either headline value crosses an epsilon.
//
// Chain mode (B3b): on top of B3, an in-memory event indexer
// (../indexer.ts) derives users / basketApr / basketEarned30d / apr30d
// from the vault's own Transfer / FeesCollected / PoolTracked event logs
// and folds them into both the priming snapshot and the 5s tick path. The
// remaining VaultSnapshot fields (allocations, pools) stay on the B1 mock
// seed values for B3b — those land in B3c via PoolRegistry + adapters.
...
// B3b indexer-derived headlines. Cached from each tick so the partial-tick
// emitter can diff them; the snapshot path always recomputes via getVaultStats
// against fresh chainTvl / chainTvl30d.
...
export async function startVaultChainReader(): Promise<void> {
  ...
  // startIndexer() succeeded. We don't flip it on here — that's a bootstrap
  // decision owned by the entrypoint.
...
// Allow index.ts to skip indexer-derived fields if startIndexer() failed or
// was not run (e.g. mock mode swap-in mid-session). Safe default: off.
...
async function readVaultPair(
  ...
  // USDC has 6 decimals on Base; shares are 18 decimals.
...
async function pollOnce(client: PublicClient, addr: `0x${string}`): Promise<void> {
  ...
  // Hook the B3b indexer onto the same 5s tick. Errors are swallowed inside
  // indexUpToHead — the chain headlines should never be blocked by indexer
  // misbehaviour.
  ...
  // intentionally not ticked outside of EOD rollover — out of scope this
  // phase.
  if (indexerEnabled && chainTvl !== null) {
    ...
    // receive it on the next snapshot (e.g. on (re)subscribe). EOD rollover
    // is out of scope this phase.
  }

  // B4 share-price-driven user re-emit. Runs every poll regardless of
  // whether the vault tick "changed" — the user-facing valueUsd recomputes
  // off live shares × live sharePrice and is debounced per-connection
  // inside reEmitOnSharePriceTick. No-op in mock mode.
