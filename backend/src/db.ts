// Persistence — single-file sqlite (bun:sqlite, WAL mode). Holds the agent
// ring + indexer state (balances, lots, activity, fee_events,
// pool_orientation, block_ts cache, first_deposit anchors,
// last_indexed_block cursor) and Sherpa per-wallet usage counters.
//
// Notes:
// - Bigints are stored as decimal strings (sqlite INTEGER is 64-bit signed).
// - Every mutation is mirrored synchronously inside the indexer's per-chunk
//   transaction (one fsync per chunk, not per event). Loads happen once at
//   boot, before backfill resumes from the saved cursor.
// - No multi-process / concurrent-writer support.
// - Schema changes after the initial CREATE block require explicit
//   ALTER TABLE statements added inline.
