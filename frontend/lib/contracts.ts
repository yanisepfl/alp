...
//
// Reads come from the backend's user.snapshot push (../backend/README.md
// "User position (B4)"), so we don't keep a parallel chain-read layer
// here. The fragments below are the minimum ABI needed for the
// deposit/redeem write path plus the allowance + balance reads that
// gate the deposit two-step flow.

import { erc20Abi } from "viem";

// Base mainnet (chainId 8453). Bridge-issued USDC at the standard
// circle.com address; CONTRACT.md §1 + DATA_INVENTORY.md fix this.
