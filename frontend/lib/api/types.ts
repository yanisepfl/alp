// ALP backend wire types.
//
// Source of truth for data-layer types consumed by the UI. Chat-
// message shapes live in `agent-stream.ts` (shared with the backend);
// re-exported here so a single edit propagates.
...
// can still be accepted. Backend convention uses this for
...
export type ClientFrame =
 ...
 // and Sherpa rate-limit accounting. Trust-on-claim — all served
 // data is derivable from public chain anyway.
 wallet?: string;
 }
 | {
 v: 1;
...
export type ApiClient = {
 ...
 // Drop the current socket and reopen. The wallet does not switch
 // mid-connection — re-subscribing with a different token while
 // already authed is logged-and-ignored, so the recovery for wallet-
 // swap and disconnect is to reconnect. agentCursor is preserved
 // across the reconnect so the user doesn't see a full history replay.
 forceReconnect(): void;
